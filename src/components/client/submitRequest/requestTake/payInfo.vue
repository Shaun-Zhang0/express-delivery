 ...
 * @Last Modified time: 2019-05-23 18:11:15
 */
<template>
  <div>
...
export default {
  ...
  methods: {
     ...
     */
    submitOrder() {
      this.$confirm("发布订单, 是否继续?", "帮我取 订单 确认", {
  ...
  mounted() {
    this.$store.commit("submitOrder", { orderType: 1 }); // 代拿订单类型
    this.$store.commit("submitOrder", { price: 0 }); // 商品价格为0，防止支付计算金额报错
  }
};
</script>
